...
<?
							...
							<div class="row-fluid">
								<div class="span12">
									...
									<div class="table table-custom">
									 ...
									 echo '</h3> 
									 <br />';
									 echo '<form action="team.php?add" id="UserAdminEditForm" enctype="multipart/form-data" method="POST" accept-charset="utf-8"><div style="display:none;"><input type="hidden" name="_method" value="PUT"/></div><fieldset>
									 
									 <div class="input text"><label for="UserDateOfBirth">სურ</label><input name="img" type="file" accept="image/*" id="UserUsername"/></div>
									 
									 <div class="input text"><label for="UserEmail">სახელი</label><input name="title" type="text" value="'.$ProductRow3['title'].'" id="UserEmail"/></div>
									 <div class="input text"><label for="UserEmail">Act</label><input style="width:750px;" name="text" type="text" value="'.$ProductRow3['text'].'" id="UserEmail"/></div>';
	...
	<div class="submit"><input  class="btn" style="margin-top: 15px;" type="submit" value="Submit"/></div></form>';
									if(isset($_POST['title']) and isset($_POST['title']))
									{
										$title = $_POST[title];
										$text = $_POST['text'];
										...
										else
										{
												...
												$AddProduct = mysql_query("INSERT INTO team(`name`,`act`) VALUES('$title','$text')");
												if($AddProduct == true)
												{
												...
												if(!empty($filep))
												{
													...
													{
														$move = move_uploaded_file($_FILES["img"]["tmp_name"], $path."/team/".$name);
														echo $move;
														$UpdateProduct = mysql_query("UPDATE team SET img='http://multibusinessgroup.ge/team/$name' WHERE name='$title' and `act`='$text'") or die(mysql_error());
													}
												}
									...
									<div class="table table-custom">
									 ...
									 <br />
										<div class="tab-content">
											Are you sure?<br> <a href="team.php?del='.$_GET[del].'&yes">Yes, of course</a><br><a href="slider.php" style="color:red;">No</a>                                    </div>
									</div>
									</div></div></div></div></div></div>';
									if(isset($_GET[yes]))
									{
										...
										if($DelProduct == true)
										{
											echo '<META HTTP-EQUIV="REFRESH" CONTENT="0; URL=team.php"><h2 style="color:green;"><b>'.$ProductRow2[name].' წაშლილია</b></h2>';
	...
	if(isset($_GET['edit']) and !empty($_GET['edit']))
	{
		...
		if(isset($_GET[d]))
		{
			...
			if($d == '1')
			{
				$Up = mysql_query("UPDATE team SET img='' WHERE id='$id'");
			}
		}
	
	$Product3 = mysql_query("SELECT * FROM team WHERE id='".$id."'");
							...
							<div class="row-fluid">
								<div class="span12">
									...
									<div class="table table-custom">
									 ...
									 <br />';
									 echo '<form action="team.php?edit='.$ProductRow3[id].'" id="UserAdminEditForm" enctype="multipart/form-data" method="POST" accept-charset="utf-8"><div style="display:none;"><input type="hidden" name="_method" value="PUT"/></div><fieldset>
									 ...
									 <div class="input text"><label for="UserEmail">სახელი</label><input name="title" type="text" value="'.$ProductRow3['name'].'" id="UserEmail"/></div>
									 <div class="input text"><label for="UserEmail">Act</label><input style="width:750px;" name="text" type="text" value="'.$ProductRow3['act'].'" id="UserEmail"/></div>';
	...
	<div class="submit"><input  class="btn" style="margin-top: 15px;" type="submit" value="Submit"/></div></form>';
									...
									if(isset($_POST['title']) and $_POST[text])
									{
										$title = $_POST[title];
										$text = $_POST['text'];
										...
										{
												$name = rand(1000,99999).rand(1000,99999).rand(1000,99999).rand(1000,99999).'.jpg';
												$UpdateProduct = mysql_query("UPDATE team SET name='$title', act='$text' WHERE id='$id'") or die(mysql_error());
												...
												if(!empty($filep))
												{
													...
													if(is_uploaded_file($_FILES["img"]["tmp_name"]))
													{
														$move = move_uploaded_file($_FILES["img"]["tmp_name"], $path."/team/".$name);
														echo $move;
														$UpdateProduct = mysql_query("UPDATE team SET img='http://multibusinessgroup.ge/team/$name' WHERE id='".$id."'") or die(mysql_error());
													}
												}
												echo '<META HTTP-EQUIV="REFRESH" CONTENT="0; URL=team.php">';
	...
	echo '<div class="row-fluid ">
				<div class="span12">
					...
					<div class="widget-body">
						...
						<a href="#">სია</a>                </li>
								<li >
						<a href="team.php?add">დამატება</a>                </li>
							</ul>
									 <br />
									 <div class="tab-content">
						...
						<table class="table table-hover">
							...
							<tr>
											...
											<a href="/slider.php/index/sort:id/direction:asc" class="desc">Id</a>                                    </th>
																		<th>
											<a href="/slider.php/index/sort:username/direction:asc">სურათი</a>                                    </th>
											<th>
											<a href="/slider.php/index/sort:username/direction:asc">სათაური</a>                                    </th>
											<th>
											<a href="/slider.php/index/sort:username/direction:asc">ACT</a>                                    </th>
						   ...
						   $Product = mysql_query("SELECT * FROM team ORDER BY id DESC") or die(mysql_error());
						   $ProductRow = mysql_fetch_array($Product);
						   do
						   {
							   echo "<tr><td class=''>".$ProductRow[id]."</td><td class=''><img src='".$ProductRow[img]."' width='200'></td><td class=''>".$ProductRow[name]."</td><td class=''>".$ProductRow[act]."</td>";

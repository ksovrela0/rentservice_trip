...
<?
							...
							<div class="row-fluid">
								<div class="span12">
									...
									<div class="table table-custom">
									 ...
									 echo '</h3> 
									 <br />';
									 echo '<form action="scategory.php?add" id="UserAdminEditForm" enctype="multipart/form-data" method="POST" accept-charset="utf-8"><div style="display:none;"><input type="hidden" name="_method" value="PUT"/></div><fieldset>
									 <div class="input text"><label for="UserEmail">კატეგორიის ID</label><input name="cat_id" type="text" value="'.$ProductRow3['cat_id'].'" id="UserEmail"/></div>
	...
	<div class="submit"><input  class="btn" style="margin-top: 15px;" type="submit" value="Submit"/></div></form>';
									if(isset($_POST['name_geo']))
									{
										...
										{
												$AddProduct = mysql_query("INSERT INTO sub_category(`name_rus`,`name_geo`,`cat_id`) VALUES('$name_rus','$name_geo','$cat_id')");
												if($AddProduct == true)
												{
												...
												echo '<META HTTP-EQUIV="REFRESH" CONTENT="0; URL=scategory.php">';
	...
	$Product2 = mysql_query("SELECT * FROM sub_cateogry WHERE id='".$id."'");
									...
									<div class="table table-custom">
									 ...
									 <br />
										<div class="tab-content">
											Are you sure?<br> <a href="scategory.php?del='.$_GET[del].'&yes">Yes, of course</a><br><a href="slider.php" style="color:red;">No</a>                                    </div>
									</div>
									</div></div></div></div></div></div>';
									if(isset($_GET[yes]))
									{
										$DelProduct = mysql_query("DELETE FROM sub_category WHERE id='".$id."'");
										if($DelProduct == true)
										{
											echo '<META HTTP-EQUIV="REFRESH" CONTENT="0; URL=scategory.php"><h2 style="color:green;"><b>'.$ProductRow2[title].' წაშლილია</b></h2>';
	...
	$Product3 = mysql_query("SELECT * FROM sub_category WHERE id='".$id."'");
							...
							<div class="row-fluid">
								<div class="span12">
									...
									<div class="table table-custom">
									 ...
									 <br />';
									 echo '<form action="scategory.php?edit='.$ProductRow3[id].'" id="UserAdminEditForm" enctype="multipart/form-data" method="POST" accept-charset="utf-8"><div style="display:none;"><input type="hidden" name="_method" value="PUT"/></div><fieldset>
									 <div class="input text"><label for="UserEmail">კატეგორიის ID</label><input name="cat_id" type="text" value="'.$ProductRow3['cat_id'].'" id="UserEmail"/></div>
									 <div class="input text"><label for="UserEmail">სახელი GEO</label><input name="name_geo" type="text" value="'.$ProductRow3['name_geo'].'" id="UserEmail"/></div>
									 <div class="input text"><label for="UserEmail">სახელი RUS</label><input name="name_rus" type="text" value="'.$ProductRow3['name_rus'].'" id="UserEmail"/></div>';
	...
	<div class="submit"><input  class="btn" style="margin-top: 15px;" type="submit" value="Submit"/></div></form>';
									...
									if(isset($_POST['name_geo']))
									{
										...
										{
												$UpdateProduct = mysql_query("UPDATE sub_category SET name_rus='$name_rus', name_geo='$name_geo', cat_id='$cat_id' WHERE id='$id'") or die(mysql_error());
	...
	echo '<div class="row-fluid ">
				<div class="span12">
					...
					<div class="widget-body">
						...
						<a href="#">სია</a>                </li>
								<li >
						<a href="scategory.php?add">დამატება</a>                </li>
							</ul>
									 <br />
									 <div class="tab-content">
					...
					<div class="widget-body">
						<table class="table table-hover">
							...
							<tr>
											...
											<th>
											<a href="/slider.php/index/sort:username/direction:asc">კატეგორიის ID / დასახელება</a>                                    </th>
						   ...
						   $Product = mysql_query("SELECT * FROM sub_category ORDER BY id DESC") or die(mysql_error());
						   $ProductRow = mysql_fetch_array($Product);
						   do
						   {
							   echo "<tr><td class=''>".$ProductRow[id]."</td><td class=''>".$ProductRow[name_geo]."</td><td class=''>".$ProductRow[cat_id]." / ";
							   $getCatNa = mysql_query("SELECT * FROM category WHERE id='$ProductRow[cat_id]' LIMIT 1");
							   $getCatNaR = mysql_fetch_array($getCatNa);
							   echo $getCatNaR[name_geo];
							   echo "</td>";

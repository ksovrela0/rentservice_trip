...
<?
							...
							<div class="row-fluid">
								<div class="span12">
									...
									<div class="table table-custom">
									 ...
									 <div class="input text"><label for="UserDateOfBirth">მთავარი საურათი</label><input name="img" type="file" accept="image/*" id="UserUsername"/></div>
	...
	<div class="submit"><input  class="btn" style="margin-top: 15px;" type="submit" value="Submit"/></div></form>';
									if(isset($_POST['name_geo']))
									{
										...
										else
										{
												...
												$AddProduct = mysql_query("INSERT INTO blog(`name_geo`,`name_rus`,`description_rus`,`description_geo`) VALUES('$name_geo','$name_rus','$desc_rus','$desc_geo')");
												if($AddProduct == true)
												{
												...
												if(!empty($filep))
												{
													...
													{
														$move = move_uploaded_file($_FILES["img"]["tmp_name"], $path."/blog/".$name);
														echo $move;
														$UpdateProduct = mysql_query("UPDATE blog SET m_pic='http://fardebibella.com/blog/$name' WHERE name_geo='$name_geo' and description_geo='$desc_geo'") or die(mysql_error());
													}
												}
									...
									<div class="table table-custom">
									 ...
									 <br />
										<div class="tab-content">
											Are you sure?<br> <a href="blog.php?del='.$_GET[del].'&yes">Yes, of course</a><br><a href="blog.php" style="color:red;">No</a>                                    </div>
									</div>
									</div></div></div></div></div></div>';
									if(isset($_GET[yes]))
									{
										...
										if($DelProduct == true)
										{
											echo '<META HTTP-EQUIV="REFRESH" CONTENT="0; URL=blog.php"><h2 style="color:green;"><b>blog '.$ProductRow2[title].' წაშლილია</b></h2>';
	...
	if(isset($_GET['edit']) and !empty($_GET['edit']))
	{
		...
		if(isset($_GET[d]))
		{
			...
			if($d == '1')
			{
				$Up = mysql_query("UPDATE blog SET m_pic='' WHERE id='$id'");
			}
							...
							<div class="row-fluid">
								<div class="span12">
									...
									<div class="table table-custom">
									 ...
									 <div class="input text"><label for="UserDateOfBirth">მთავარი საურათი(<a href="?edit='.$id.'&d=1" style="color:red;">წაშლა</a>)</label><input name="img" type="file" accept="image/*" id="UserUsername"/></div>
	...
	<div class="submit"><input  class="btn" style="margin-top: 15px;" type="submit" value="Submit"/></div></form>';
									...
									if(isset($_POST['name_geo']))
									{
										...
										{
												$UpdateProduct = mysql_query("UPDATE blog SET name_geo='$name_geo', name_rus='$name_rus', description_rus='$desc_rus', description_geo='$desc_geo' WHERE id='$id'") or die(mysql_error());
												...
												if(!empty($filep))
												{
													...
													if(is_uploaded_file($_FILES["img"]["tmp_name"]))
													{
														$move = move_uploaded_file($_FILES["img"]["tmp_name"], $path."/blog/".$name);
														echo $move;
														$UpdateProduct = mysql_query("UPDATE blog SET m_pic='http://fardebibella.com/blog/$name' WHERE id='".$id."'") or die(mysql_error());
													}
												}
	...
	echo '<div class="row-fluid ">
				<div class="span12">
					...
					<div class="widget-body">
						...
						<a href="#">სია</a>                </li>
								<li >
						<a href="blog.php?add">დამატება</a>                </li>
							</ul>
									 <br />
									 <div class="tab-content">
					...
					<div class="widget-body">
						<table class="table table-hover">
							...
							<tr>
											...
											<th>
											<a href="/users.php/index/sort:username/direction:asc">დასახელება</a>                                    </th>
											<th>
											<a href="/users.php/index/sort:username/direction:asc">აღწერა</a>                                    </th>                         
						   ...
						   do
						   {
							   ...
							   echo $ProductRow[name_geo];
							   echo "</td><td class=''>".$ProductRow[description_geo]."</td>";		
